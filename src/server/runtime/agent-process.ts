...
export class CodexAgentProcess {
  ...
  async runTurn(objective: string, transcript: string, triggerSummary: string): Promise<AgentTurnResult> {
    ...
    const turnPrompt = [
      ...
      ...routingGuidanceLines(this.agent, this.config.agents),
      ...sharedTurnProtocolLines({ hasSessionStateTools: this.hasSessionStateTools() }),
      "Discussion-first reminder:",
      "- Non-owner same-card findings belong in append_subgoal_discussion first.",
      "- Owner or coordinator turns should materialize durable findings onto the card with subgoalUpdates so facts/open/resolved/acceptance/files do not stay empty.",
      "- If your finding is actually a different durable axis, create a new subgoal instead of forcing it into the current card.",
      "- If the same contract is already on the board under a slightly different title or topicKey, reuse or merge that card instead of spawning a renamed duplicate.",
      "Return exactly this shape between the XML tags:",
      "<codex_research_team-response>",
      '{"shouldReply":true,"workingNotes":["short public note"],"teamMessages":[{"targetAgentId":"coordinator_1","subgoalIds":["sg-runtime-bootstrap"],"content":"routing or handoff message when another agent must act now"}],"subgoalUpdates":[{"title":"Runtime bootstrap contract","topicKey":"runtime-bootstrap","summary":"current canonical contract","addFacts":["durable fact"],"addOpenQuestions":["remaining question"],"addResolvedDecisions":["resolved point"],"addAcceptanceCriteria":["acceptance check"],"addRelevantFiles":["src/example.ts"],"nextAction":"next canonical step","stage":"researching","decisionState":"disputed"}],"completion":"continue"}',
      "</codex_research_team-response>",
