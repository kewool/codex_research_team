export function createPageRenderers(deps) {
    ...
    function renderDashboardSessionCard(session) {
        const waiting = (session.agents || []).filter((agent) => agent.waitingForInput).length;
        const errors = (session.agents || []).filter((agent) => agent.status === "error").length;
    ...
    function renderWorkspaceCreateModal() {
    ...
    function renderWorkspacesPage() {
        ...
        const workspaceList = (config.workspaces || []).map((workspace) => {
      ...
      `;
        ...
        const workspaceEditors = (config.workspaces || []).map((workspace, index) => {
    ...
    function renderSettingsPage() {
        ...
        const agentIds = (config.agents || []).map((agent) => String(agent.id || "").trim()).filter(Boolean);
        ...
        const agentRows = (config.agents || []).map((agent, index) => `
      <article class="agent-editor-card">
        ...
        <div class="two-col-grid">
          <label>
            ...
            ${renderAgentCheckboxPicker("data-agent-target-allow-option", index, agentIds.filter((value) => value !== String(agent.id || "").trim()), Array.isArray(agent.policy?.allowedTargetAgentIds) ? agent.policy.allowedTargetAgentIds : [])}
          </label>
          <label>
