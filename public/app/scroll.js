export function createScrollTools(state) {
    function saveElementScrollAnchor(element, modeOverride) {
    ...
    function captureRenderScrollSnapshot() {
        ...
        document.querySelectorAll("[data-scroll-key]").forEach((element) => {
            ...
            const saved = state.sessionScroll.anchors[key];
            if (saved) {
                anchors[key] = { ...saved };
                return;
            }
    ...
    function restoreRenderScrollSnapshot(snapshot) {
    ...
    function syncSessionScrollMemoryFromDom() {
        if (state.route.name !== "session") {
            return;
        }
    ...
    function bindSessionScrollMemory() {
        if (state.route.name !== "session") {
            return;
        }
        syncSessionScrollMemoryFromDom();
