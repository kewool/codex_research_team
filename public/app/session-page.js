export function createSessionPageRenderers(deps) {
    const { state, escapeHtml, cleanTerminalText, feedCache, agentHistoryCache, currentAgentTab, historyKindForTab, currentSession, ensureSelectedAgent, ensureSessionPageData, formatTokenUsage, formatTokenCount, tokenValue, renderLabel, } = deps;
    function renderListFooter(cache, emptyText) {
    ...
    function renderHistoryEntry(entry, tab) {
    ...
    function renderFeedItem(event) {
        ...
        const targetAgentIds = Array.isArray(meta.targetAgentIds)
            ? meta.targetAgentIds.map((value) => String(value ?? "").trim()).filter(Boolean)
            : meta.targetAgentId
        ...
        const subgoalIds = Array.isArray(meta.subgoalIds)
            ? meta.subgoalIds.map((value) => String(value ?? "").trim()).filter(Boolean)
            : [];
    ...
    function renderSessionFeed(sessionId) {
    ...
    function renderSubgoalBoard(session) {
        ...
        const renderMemoryList = (label, items, emptyText) => {
        ...
        const renderConflictHistory = (entries) => {
      ...
      `;
        ...
        const renderDiscussionEntries = (entries) => {
      ...
      ${activeSubgoals.length > 0 ? `
        <div class="subgoal-board" data-subgoal-count="${escapeHtml(String(activeSubgoals.length))}" data-scroll-key="${escapeHtml(`subgoal-board:${session.id}:active`)}" data-scroll-mode="append">
          ${activeSubgoals.map((subgoal) => renderSubgoalCard(subgoal)).join("")}
        </div>
    ...
    function renderAgentTabContent(sessionId, agent, tab) {
        if (tab === "stdout") {
            return `
    ...
    function renderAgentPickerItem(agent) {
    ...
    function renderFocusedAgentCard(session, agent) {
    ...
    function renderSessionPage() {
      ...
      <section class="summary-row">
        ...
        <article class="metric-card panel"><span class="metric-label">Subgoals</span><strong>${escapeHtml(String((session.subgoals || []).filter((subgoal) => !subgoal.mergedIntoSubgoalId && !subgoal.archivedAt).length))}</strong></article>
